...
# load data
train = pd.read_csv("../data/wsdm_competition/wsdm_data.csv")
...
######################### load extra data and do the same thing like train ###############################
train2 = pd.read_csv('../data/wsdm_competition/lmsys.csv')
...
train2["input_ids"], train2["attention_mask"] = tokenize(tokenizer, train2["prompt"], train2["response_a"], train2["response_b"],template_1tokenized,template_2tokenized,template_3tokenized,template_4tokenized,template_5tokenized,template_6tokenized,template_7tokenized,MAX_LENGTH,MAX_PROMPT_LENGTH)
train2['text'] = train2['input_ids'].apply(lambda x: tokenizer.decode(x))
...
print(train_reverse2['label'].value_counts())

train_reverse2["input_ids"], train_reverse2["attention_mask"] = tokenize(tokenizer, train_reverse2["prompt"], train_reverse2["response_b"], train_reverse2["response_a"],template_1tokenized,template_2tokenized,template_3tokenized,template_4tokenized,template_5tokenized,template_6tokenized,template_7tokenized,MAX_LENGTH,MAX_PROMPT_LENGTH)
train_reverse2['text'] = train_reverse2['input_ids'].apply(lambda x: tokenizer.decode(x))
...
for fold in range(5):
    ...
    with open(f"../data/processed_data/{save_name}fold{fold}/dev.pkl", 'wb') as f:
        pickle.dump(dev, f)
    with open(f"../data/processed_data/{save_name}fold{fold}/train.pkl", 'wb') as f:
        pickle.dump(tra, f)
